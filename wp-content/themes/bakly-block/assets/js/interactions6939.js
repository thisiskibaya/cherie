...
( function () {
	...
	function initSearchOverlay() {
		...
		input.addEventListener( 'input', function () {
			...
			timer = window.setTimeout( function () {
				...
				fetch( '/wp-json/wc/store/v1/products?search=' + encodeURIComponent( q ) + '&per_page=6&catalog_visibility=visible', { signal: controller.signal } )
					.then( function ( r ) {
						return r.ok ? r.json() : [];
					} )
					.then( function ( items ) {
	...
	function initMobileDrawer() {
		var btn = document.querySelector( '.bakly-drawer-toggle' );
		var panel = document.getElementById( 'bakly-mobile-drawer' );
		if ( ! btn || ! panel || btn.dataset.baklyDrawerBound ) {
			return;
		}
		btn.dataset.baklyDrawerBound = '1';
		...
		var closeBtn = panel.querySelector( '.bakly-drawer-close' );
